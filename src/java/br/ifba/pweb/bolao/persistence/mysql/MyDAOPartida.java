 ...
 * @author lisy
 */
public class MyDAOPartida implements IDAOPartida{
    ...
    public void salvar(Partida p) throws Exception {
        String sql="INSERT INTO `partida` (`fase`,`sede_id`,`nacao_id`,`nacao_id1`,`data`,`horario`) VALUES " +
"(?,?,?,?,?,?)";
        try{
        ...
        try{
         ...
         stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
             while(rs.next()) {
                 IDAONacao ndao= new MyDAONacao();
                 IDAOSede sdao = new MyDAOSede();
                 
              p= new Partida(rs.getString("`fase`"), sdao.recuperarpeloId(rs.getInt("`sede_id`")),ConvesorData.DateToCalendar(rs.getDate("`data`")),rs.getTime("`horario`"),ndao.recuperarPeloId(rs.getInt("`nacao_id`")), ndao.recuperarPeloId(rs.getInt("`id_adversaria`")));
              p.setId(id);
         ...
         stmt.setString(1, fase);
            ResultSet rs = stmt.executeQuery();
             while(rs.next()) {
              ...
              p.add(new Partida(rs.getString("`fase`"), sdao.recuperarpeloId(rs.getInt("`sede_id`")),ConvesorData.DateToCalendar(rs.getDate("`data`")),rs.getTime("`horario`"),ndao.recuperarPeloId(rs.getInt("`nacao_id`")), ndao.recuperarPeloId(rs.getInt("`id_adversaria`"))));
              for (Partida pa:p){
